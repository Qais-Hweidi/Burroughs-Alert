...
async function testURLLength() {
  ...
  try {
    // Get alert 21
    const result = await db.select().from(alerts).where(eq(alerts.id, 21));
    ...
    console.log(
      '   First 200 chars:',
      neighborhoodsParam.substring(0, 200) + '...'
    );
    console.log(
      '   Last 100 chars: ...',
      neighborhoodsParam.substring(neighborhoodsParam.length - 100)
    );
  } catch (error) {
