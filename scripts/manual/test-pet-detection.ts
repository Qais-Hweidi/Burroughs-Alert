...
async function testPetDetection() {
  ...
  try {
    ...
    const url = 'https://newyork.craigslist.org/brk/apa/d/brooklyn-williamsburg-beautiful-bedroom/7861257104.html';
    ...
    const data = await page.evaluate(() => {
      const title = document.querySelector('h1.postingtitle')?.textContent?.trim() || '';
    ...
    console.log(`Title + Description: ${detectPetFriendly(data.title, data.fullDescription)}`);
    console.log(`Title + Housing Attrs: ${detectPetFriendly(data.title, data.housingText)}`);
    console.log(`Title + All Attrs: ${detectPetFriendly(data.title, data.allAttributesText)}`);
    ...
    patterns.forEach((pattern, index) => {
      ...
      console.log(`Pattern ${index + 1} (${pattern}): ${match ? `MATCH: "${match[0]}"` : 'NO MATCH'}`);
    });
